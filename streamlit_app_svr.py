...
def r2_score(y_true: np.ndarray, y_pred: np.ndarray) -> float:
    ...
    return 1.0 if ss_tot == 0.0 else float(1.0 - (ss_res / ss_tot))
...
def train_svr(X: np.ndarray, y: np.ndarray, *, kernel: str, C: float, gamma: str, epsilon: float):
    try:
        ...
        from sklearn.svm import SVR  # type: ignore[reportMissingModuleSource]
    except ImportError as e:
        raise ImportError("scikit-learn is required to train SVR.") from e
